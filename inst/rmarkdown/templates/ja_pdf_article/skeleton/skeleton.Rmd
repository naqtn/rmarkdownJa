...
（日本組版のための BXjscls パッケージ で提供されている BXJS 文書クラス）
* bxjsarticle ： 章のないレポート
* bxjsreport ： 章のあるレポート
* bxjsbook ： 書籍
* bxjsslide ： スライド （使用可だが beamer_presentation を使ったほうが良いだろう）

classoption でフォントサイズ、用紙サイズ、landscape などのパラメータを指定可能。
詳細は
...
（BXJS 文書クラスが内部的に使うため。上記マニュアルを参照。）

TeX エンジンに LuaTeX を使いたい場合には３箇所の xelatex との部分を lualatex にする。
...
if (knitr::opts_knit$get("rmarkdown.pandoc.to") %in% c("beamer", "latex")) {
  ...
  ## 2. cairo_pdf を使う方法
  # * family には Windows にインストールされているフォント名を指定する。
  # * ここでは例として Meiryo を指定。（コントロールパネル > フォント 参照）
